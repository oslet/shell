...
if [ ! -f /sys/devices/syste/node/node0/cpu0/online ]; then
...
function showcpus()
{
    ...
    do
        grep . /sys/devices/system/node/${node}/cpu*/online /dev/null | cut -d"/" -f6- | sed s/"\/online"/""/g | sed s/":1$"/" is Online"/g | sed s/":0$"/" is Offline"/g
done
}
